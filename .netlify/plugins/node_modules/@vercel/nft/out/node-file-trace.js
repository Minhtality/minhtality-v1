...
class Job {
    ...
    readlink(path) {
        ...
        if (cached !== undefined)
            return cached;
        try {
            const link = fs_1.default.readlinkSync(path);
    ...
    emitFile(path, reason, parent, isRealpath = false) {
        if (!isRealpath)
    ...
    getPjsonBoundary(path) {
        ...
        while ((separatorIndex = path.lastIndexOf(path_1.sep)) > rootSeparatorIndex) {
            ...
            if (this.isFile(path + path_1.sep + 'package.json'))
                return path;
        }
        return undefined;
    }
    async emitDependency(path, parent) {
        ...
        if (path.endsWith('.js')) {
            ...
            if (pjsonBoundary)
                this.emitFile(pjsonBoundary + path_1.sep + 'package.json', 'resolve', path);
        }
        let analyzeResult;
