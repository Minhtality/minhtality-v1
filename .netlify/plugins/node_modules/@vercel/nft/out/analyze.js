...
async function analyze(id, code, job) {
    ...
    function computePureStaticValue(expr, computeBranches = true) {
        ...
        const result = static_eval_1.evaluate(expr, vars, computeBranches);
        return result;
    }
    ...
    function processRequireArg(expression, isImport = false) {
    ...
    special_cases_1.default({ id, ast, emitAsset: path => assets.add(path), emitAssetDirectory, job });
    function backtrack(parent, context) {
                ...
                // handle well-known function symbol cases
                    ...
                    switch (calleeValue.value) {
                        ...
                        case PKG_INFO:
                            ...
                            while (pjsonPath !== rootPjson && !fs_1.existsSync(pjsonPath))
    ...
    function emitAssetPath(assetPath) {
        ...
        try {
            var stats = fs_1.statSync(basePath);
        }
        catch (e) {
            return;
        }
    ...
    function emitStaticChildAsset() {
        ...
        if ('value' in staticChildValue && isAbsolutePathStr(staticChildValue.value)) {
            ...
            if (resolvedThen)
                emitAssetPath(resolvedThen);
            if (resolvedElse)
                emitAssetPath(resolvedElse);
        }
