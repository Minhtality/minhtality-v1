        ...
        if (id.endsWith('socket.io/lib/index.js')) {
            function replaceResolvePathStatement(statement) {
                if (statement.type === 'ExpressionStatement' &&
                    ...
                    let resolved;
                    try {
                        const dep = resolve_dependency_1.default(arg, id, job);
...
function handleSpecialCases({ id, ast, emitAsset, emitAssetDirectory, job }) {
    const pkgName = get_package_base_1.getPackageName(id);
    const specialCase = specialCases[pkgName || ''];
    id = id.replace(/\\/g, '/');
    if (specialCase)
        specialCase({ id, ast, emitAsset, emitAssetDirectory, job });
}
